...
killings.groupby('state').size().order().tail(5)
...
majors.groupby('Major').Median.mean().order().tail(10)
# 4. Plot the data from the last question in a bar chart, include proper title, and labels!
majors.groupby('Major').Median.mean().order().tail(10).plot(kind='bar',title='Top 10 Highest Paying Majors')
...
majors.groupby('Major_category').Unemployment_rate.order().tail(10)
